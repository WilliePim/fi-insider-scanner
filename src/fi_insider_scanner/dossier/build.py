"""Caso zero (ADR-038): selezione dichiarata e dossier WHO / WHERE / WHEN, senza verdetto.

Il dossier contiene solo fatti con fonte: registro FI (righe citate per record_id), dati di mercato
(Yahoo, dichiarati), fonti primarie lette dall'analista e passate in un file JSON di fonti
(`dossier/<caso>_sources.json`: fatti con URL e data). Tutto ciò che non è stato letto va in
NON VERIFICATO / MISSINGNESS. Il punteggio WHO/WHERE/WHEN è provvisorio (asterisco) e le sue ancore
sono scritte nel dossier stesso.
"""
...
BUY_KINDS = {"acq_purchase", "subscription", "grant", "exercise_in", "gift_in", "conversion_in", "exchange_in", "inheritance_in", "division_in", "merger_in", "internal_in"}
SELL_KINDS = {"disp_sale", "exercise_out", "gift_out", "conversion_out", "exchange_out", "inheritance_out", "division_out", "merger_out", "internal_out", "redemption"}
...
def select_case(events_b: pd.DataFrame, cfg: dict) -> CaseSelection:
    ...
    rule = (f"trigger B con T in [{cz['window_start']}, {cz['window_end']}], score 4/4, non stale, banda 50-300M USD as-of l'ancora, "
...
def _person_block(v_all: pd.DataFrame, v_at_t: pd.DataFrame, person: str, window: pd.DataFrame, as_of: pd.Timestamp, cfg: dict, shares_out: float | None) -> str:
    ...
    ub = f"≤ {vol / lb:.1%} (limite superiore: posizione minima visibile nel registro {lb:,.0f} azioni)" if lb > 0 else "UNKNOWN (nessuna posizione visibile nel registro prima del cluster: il registro parte dal 2016-07 e non contiene il possesso)"
    ...
    lines.append(f"- Storia 5 anni nel registro (questo emittente): {len(buys)} righe in aumento ({', '.join(sorted(set(buys['txn_kind'])) ) or '—'}), "
    ...
    lines.append(f"- Routine a T: mesi con acquisti (12m) {m.n_buy_months}, dispersione {'—' if m.dispersion is None else f'{m.dispersion:.2f}'} → {'routine' if m.is_routine else 'non routine'}; etichetta CMP: {m.cmp_label}.")
    lines.append(f"- Grande azionista: {large_holder(v_at_t, person, shares_out, cfg['large_holder']['position_share_threshold'])} (mai `false`: MAR non copre i >10%).")
    return "\n".join(lines) + "\n"


def build_dossier(case: pd.Series, register: Register, cfg: dict, shares_out: float | None, report_dates: pd.DatetimeIndex | None, sources: dict | None, in_band: bool, rule: str, snapshot_note: str) -> str:
    ...
    w("## Testata\n")
    head = [
        ("Emittente", name), ("LEI / issuer_key", issuer), ("ISIN", case["isin"]), ("Ticker (verificato sui prezzi del registro)", case["symbol"]),
        ("T (prima pubblicazione che completa il cluster)", str(as_of)), ("Ancora (prima transazione)", str(pd.Timestamp(case["anchor"]).date())),
        ("Ultima transazione della finestra", str(pd.Timestamp(case["window_end"]).date())),
        ("Market cap as-of ancora", f"{_fmt_sek(case['mcap_sek'])} SEK = {case['mcap_usd']/1e6:,.0f} M USD (azioni {_fmt_sek(case['shares_used'])} al {str(case['shares_date'])[:10]} da Yahoo, prezzo {case['price_used']} da {case['price_source']}, USDSEK {case['usdsek']:.3f})" if pd.notna(case["mcap_usd"]) else "UNKNOWN"),
        ("Banda", case["band"] or "UNKNOWN"),
        ("Persone fisiche distinte", int(case["n_persons"])), ("Valore SEK del cluster", _fmt_sek(case["value_sek"])),
    ...
    l1 = [
        ...
        ("dilution veto ≠ BLOCKED", bool(case["dilution_ok"]), f"verdetto {case['dilution']}, partecipazione {bool(case['dil_participation'])}, trappola {bool(case['dil_trap'])}, crescita azioni {'—' if pd.isna(case['dil_growth']) else f'{case['dil_growth']:+.1%}'}"),
        ("nessun grande azionista `true`", bool(case["no_large_holder"]), f"n_large_holder = {int(case['n_large_holder'])}"),
        ("S3 (sottoscrizione strutturale) nella finestra", bool(case["s3_in_window"]), f"prezzo uniforme on-venue: {bool(case['uniform_onvenue'])}; S1 {bool(case['s1_subscription'])}, S2 {bool(case['s2_issue_instruments'])}"),
    ]
    ...
    later = v_all[base_purchase(v_all) & (v_all["published_at"] > as_of) & (v_all["trade_date"] <= pd.Timestamp(case["anchor"]) + pd.Timedelta(days=cfg["cluster"]["window_days"])) & ~v_all["person_key"].isin(persons)]
    if not later.empty:
    ...
    w(md_table(pd.DataFrame([("WHO", f"{who}/3*" if who is not None else "n/d*"), ("WHERE", f"{where}/3*" if where is not None else "n/d*"),
                              ("WHEN", f"{when_s}/3*" if when_s is not None else "n/d*"), ("Totale", f"{tot}/9*" if tot is not None else "n/d*")], columns=["dimensione", "punteggio"])))
    if sc.get("notes"):
