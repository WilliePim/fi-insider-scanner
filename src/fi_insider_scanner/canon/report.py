"""Checkpoint 2 report: the canonical table."""
...
def build_report(c: Canonical, snapshot_id: str) -> str:
    ...
    w(f"Generato {datetime.now(UTC).isoformat(timespec='seconds')} dallo snapshot `{snapshot_id}`.\n")
    ...
    w("\n## Tassonomie\n")
    w(
        f"Karaktär non mappati: **{int((df['txn_kind'] == 'unmapped').sum())}**. "
    ...
    w(
        md_table(
            df.loc[df["venue_class"] == "other_venue", "venue_raw"]
            .value_counts()
            .head(20)
            .rename_axis("Handelsplats")
            .reset_index(name="n")
        )
    )
    ...
    if not s.links.empty:
        ...
        for r in sample.itertuples():
            ...
            rows.append(
                (
                    cor["issuer_name_raw"][:28],
                    cor["pdmr_name"][:22],
                    str(cor["trade_date"].date()),
                    str(pre["published_at"]),
                    str(cor["published_at"]),
                    pre["status_raw"],
                    r.score,
                    (r.note or "")[:50],
                )
        ...
        w(
            md_table(
                pd.DataFrame(rows, columns=["emittente", "persona", "trade", "pub prec.", "pub corr.", "status prec.", "score", "nota"])
            )
        )
        ...
        md_table(
            pd.DataFrame(
                [
                    ...
                    (
                        "flag NEAR_DUP_NAME (solo flag)",
                        int((c.name_flags["flag"] == "NEAR_DUP_NAME").sum()) if not c.name_flags.empty else 0,
                    ),
                ],
                columns=["voce", "n"],
            )
                ...
                pd.DataFrame(
                    [
                        (
                            ...
                            r.long_key,
                            r.short_key,
                            str(r.valid_from.date()),
                            "" if r.valid_until is None else str(r.valid_until.date()),
                        )
                        for r in sample
                    ],
