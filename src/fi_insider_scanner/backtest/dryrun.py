"""Checkpoint 3: dry-run degli eventi con soli dati del registro e FX (nessun prezzo azionario)."""
...
def near_dup_persons(b: pd.DataFrame) -> pd.Series:
    """True se due persone del cluster coincidono a meno dei diacritici (possibile doppio conteggio)."""
...
def build_report(cfg: dict, a: pd.DataFrame, b: pd.DataFrame, b_seen: pd.DataFrame, b_first: pd.DataFrame) -> str:
    ...
    w(f"Generato {datetime.now(timezone.utc).isoformat(timespec='seconds')}. Nessun prezzo azionario usato. "
